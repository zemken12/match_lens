...
def build_figure(selected_players, half, minute_range):
    ...
    for team in teams:
        ...
        for shot in shots:
            ...
            # Flip second team so teams attack opposite goals visually
            if team == teams[1]:
                x = 120 - x
                y = 80 - y
    ...
    fig.update_layout(
        title="Match Lens - Interactive Shot Map",
        width=1100,
        height=750,
        plot_bgcolor="white",
        showlegend=True,
...
for team in teams:
    for player in players_by_team[team]:
        all_player_options.append(
            {"label": f"{team} — {player}", "value": player}
        )
...
def update_chart(selected_players, half, minute_range):
    return build_figure(selected_players, half, minute_range)
